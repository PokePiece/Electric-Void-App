...
def call_portfolio_general_chatbot(prompt: str, max_tokens: int):
    ai_response = call_chat_model(portfolio_prompt, prompt, max_tokens)
    return ai_response


def call_general_chatbot(prompt: str, max_tokens: int):
    ai_response = call_chat_model(scomaton_prompt, prompt, max_tokens)
    return ai_response


def call_brain_interface(prompt: str, max_tokens: int):
    ai_response = call_chat_model(void_interface_prompt, prompt, max_tokens)
    return ai_response


def call_portfolio_accomplishments(prompt: str, max_tokens: int):
    ai_response = call_chat_model(portfolio_accomplishments_prompt, prompt, max_tokens)
    return ai_response


def call_portfolio_masterpiece(prompt: str, max_tokens: int):
    ai_response = call_chat_model(portfolio_masterpiece_prompt, prompt, max_tokens)
    return ai_response


def call_portfolio_skills(prompt: str, max_tokens: int):
    ai_response = call_chat_model(portfolio_skills_prompt, prompt, max_tokens)
    return ai_response


def call_portfolio_reach(prompt: str, max_tokens: int):
    ai_response = call_chat_model(portfolio_reach_prompt, prompt, max_tokens)
    return ai_response
...
def call_void_general(prompt: str, max_tokens: int, user_id: str):
    ai_response = call_chat_model(void_general_prompt, prompt, max_tokens)
    return ai_response

def call_webtrix_general(prompt: str, max_tokens: int, user_id: str):
    # This now correctly receives only the ai_response from call_chat_model
    ai_response = call_chat_model(webtrix_general_prompt, prompt, max_tokens)
    return ai_response

def call_webtrix_expert(prompt: str, max_tokens: int, user_id: str):
    # This now correctly receives only the ai_response from call_chat_model
    ai_response = call_chat_model(webtrix_expert_prompt, prompt, max_tokens)
    return ai_response
...
def call_old_chat_model(system_prompt: str, prompt: str, max_tokens: int):
    headers = {
...
def call_saving_chat_model(system_prompt: str, prompt: str, max_tokens: int, tag: str, user_id: str):
    ...
    history = get_recent_messages(tag, user_id, limit=10)  
